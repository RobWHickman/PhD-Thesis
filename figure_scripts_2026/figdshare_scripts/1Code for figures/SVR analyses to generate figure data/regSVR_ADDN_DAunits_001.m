...
clear  Mdl  thisPoint  thisGroup  allOtherPoints  allOtherGroups  SVR  p  nPoints  YFit
nPoints = size(WINDOW_DATA,1) / 10; % because we use 90% training vs 10% testing of the data
pointMatrix = WINDOW_DATA;  trueGroups = GROUP_DATA;
% in case of 10 ranges of BIDS and 10 'TRAIAL_NUMBER' we get 'nPoints=100'; % we perform the regression 100 times according to the leave-one-out method
for p = 1:2:nPoints % iterate over points
    ...
    SVR(1:size(YFit,1),p) = thisGroup;  SVR(1:size(YFit,1),p+1) = YFit;
    
    close all hidden
end % for p = 1:nPoints

% create cell matrix where in columns we have results for time windows
Z{repeats,win,ADDN} = SVR;
