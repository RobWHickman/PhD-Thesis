...
%                     QuickRasterPeth(trial_rast)
                    figure
                    subplot(7,1,1:5)                    
                    PlotTrueRaster(trial_rast)
                    hold on
                    for iA= 1:length(artifact)
                        if artifact(iA)>0
                            plot(artifact(iA)+ofst,iA-.5,'LineStyle','none','Marker','x')
                        end
                    end
                    subplot(7,1,6:7)
                    plot(nanmean(smoothdata(trial_rast,2,'gaussian',50)))
                    ca
                end
% % %                 art_ctr = 0;
% % %                 for iA= 1:length(artifact)
% % %                     art_ctr = art_ctr+trial_rast(iA,artifact(iA)+pre+ofst);
% % %                 end
% % %                 if art_ctr>=3
% % %                     figure
% % %                     subplot(7,1,1:5)                    
...
% % %                     for iA= 1:length(artifact)
% % %                         if artifact(iA)>0
% % %                             plot(artifact(iA)+pre+ofst,iA-.5,'LineStyle','none','Marker','x')
% % %                             
% % %                             inc=ofst;arti=0;reentry=1;
% % %                             while arti==0
% % %                                 if trial_rast(iA,artifact(iA)+pre+inc)
% % %                                     trial_rast(iA,artifact(iA)+pre+inc)=0;
% % %                                     arti=1;
% % %                                 end
% % %                                 if inc<=3 && reentry==1 && arti==0
% % %                                     inc = inc+1;
% % %                                 elseif inc>=1 && arti==0
% % %                                     reentry=0;
% % %                                     inc=inc-1;
...
% % %                     end
% % %                     subplot(7,1,6:7)
% % %                     plot(mean(smoothdata(trial_rast,2,'gaussian',50)))
% % %                             
% % %                     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % % 
% % %                     figure
...
% % %                     for iA= 1:length(artifact)
% % %                         if artifact(iA)>0
% % %                             plot(artifact(iA)+pre+2,iA-.5,'LineStyle','none','Marker','x')
% % %                         end
% % %                     end
% % %                     subplot(7,1,6:7)
% % %                     plot(mean(smoothdata(trial_rast,2,'gaussian',50)))
...
% % %                 end                
    ...
    if  ~bad && frwEv == 0  && frw ||...
        ...
        for iB = 1:length(gdbits)
            ...
            LMH(ctr).event.previouswinlose = nan; LMH(ctr).event.previoustotalliquid = nan; LMH(ctr).event.match = nan;
            ...
            for iGx = 1:length(sDS)%for each trial defined by goodix.
                ...
                [trial_rast(iGx,:),bc] = TrialRaster(spks_per_trial,alignment,pre,post,bin);%generate a single-trial raster
                cont_trial_rast(iGx,:) = TrialRaster(spks_per_trial,cont_alignment,pre,post,bin);%generate a single-trial control raster
                joystick_rast(iGx,:) = PeriEventTraceSingleTrial(js_trace,alignment,pre,5000,1000);
            end
                        %             figure
            %             bd = double([sDS.(gdbits{iB})])-1947;
            %             col = lines(93);
            %             for i = 1:93
            %                 plot(joystick_rast(i,:)+i*100,'Color',col(i,:))
            %                 kix = find(~isnan(joystick_rast(i,:)),1,'first');
            %                 js = joystick_rast(i,kix)+i*100;
            %                 line([bd(i) bd(i)],[js-100 js+100],'Color',col(i,:))
            %                 hold on
            %             end
            %             ca
...
%                         cont = (sum(sctrz(:,(pre-cont_comp)/bin:pre/bin),2)/cont_comp)*1000;
                ...
                LMH(ctr).FR.(gdbits{iB}) = out(:,1); % early response
                if numComp>1
                    LMH(ctr).FR_late.(gdbits{iB}) = out(:,2); % late response
                end                
                LMH(ctr).FR_control.(gdbits{iB}) = cont;
