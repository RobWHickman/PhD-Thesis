function [LMH]=GettySituationDifferencesWithClusters(data_file_path_name,situations,bits,trials,win_lose_both)
...
% 
    ...
    if sum(h)>0 %& stats(3)<.05 %%%%%%%%%%%%%%%%%%%%%
        ctr = ctr+1;
        gdbits = bits(cellfun(@isempty,strfind(bits,'FreeRewardUp')));
        for iB = 1:length(gdbits)%for each relevant bit
            gis = ismember([DS.situation],[1:3])&ismember([DS.Win],[wlb{:}]);
            gx = goodix&gis;
            ...
            zFRrast = Z_scores_DH(trial_rast,[1:(300/bin)]);
            z_cont_rast = Z_scores_DH(cont_trial_rast,[1:(300/bin)]);
            ...
            for iSC = 1:length(sit_cell)                
                ...
                [r,~] = size(sitFRrast);                
                sits = [sits;repmat(iSC,[r,1])];
                sit_cont_rast = cont_rast_FR(sit_cell{iSC},:);
                FR_rast_lmh{iSC} = sitFRrast;
            end
            
            s = strfind(pth,'\');
            LMH(ctr).(gdbits{iB}) = FR_rast_lmh;
            LMH(ctr).cluster = cln(1:7);
            LMH(ctr).situations = sits;
