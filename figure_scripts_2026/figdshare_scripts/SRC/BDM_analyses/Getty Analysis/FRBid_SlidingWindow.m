...
%% Sliding Window
clearvars -except RES sigix bin cc1 cc2 testBit monk regression
% ca
pre = 2000;
bin=1;
npre = 500;
    ...
    for iB = 1:length(testBit)
        for irg = 1:length(regressors)
            ...
            for iWl = 1:length(WL)
                ...
                stix = ismember(sits,testSit) & ismember(wlix,wll{iWl});
                stiv = iv(stix);
                stst = sits(stix);
                ...
                for ii = 1:nitr
                    ...
                    if isnan(stats(1)) || stats(1)==0
                        regression.(bt).(reg).(wlb{iWl}).r2(iR,ii) = 0;
                        regression.(bt).(reg).(wlb{iWl}).pv(iR,ii) = 1;
                        regression.(bt).(reg).(wlb{iWl}).bta(iR,ii) = 0;
                    else
                        regression.(bt).(reg).(wlb{iWl}).r2(iR,ii) = stats(1);
                        regression.(bt).(reg).(wlb{iWl}).pv(iR,ii) = stats(3);
                        regression.(bt).(reg).(wlb{iWl}).bta(iR,ii) = BetaNormalization(b(2),iv,fr);
                    end
                end
            end
        end
...
for iB = 5:length(testBit)
    for irg = 1:length(regressors)
        bt = testBit{iB};
        reg = regressors{irg};
        r2 = regression.(bt).(reg).(wlb).r2;%(sigix,:);
        pv = regression.(bt).(reg).(wlb).pv;%(sigix,:);
        bta = regression.(bt).(reg).(wlb).bta;%(sigix,:);
...
for iB = 5:length(testBit)
    for irg = 1:length(regressors)
        bt = testBit{iB};
        reg = regressors{irg};
        r2 = regression.(bt).(reg).(wlb).r2;
        pv = regression.(bt).(reg).(wlb).pv;
        bta = regression.(bt).(reg).(wlb).bta;
        xax = ((1:length(r2(1,:)))*iter)-npre;
        yax = 1:height(r2);
        ...
        [sspv,srtix] = SortByPeakTime(spv,[npre/iter:(npre+500)/iter],'trough',1);
        figure
        spv_ix = sspv;
        spv_ix(spv_ix>0.010)=1;%spv_ix(spv_ix<=0.05)=0;
        % sigix_sw = ~logical(spv_ix)&bta>0;
        imagesc(xax,yax,spv_ix);%,[0 .05])+
        %         imagesc(xax,yax,pv,[0 .05])
        ...
        g.YDir = 'normal';
        colorbar
        title(['p value | ',bt,' | ',reg])


        % ssr2 = smoothdata(r2(srtix,:),1,'movmean',100);
        % ssr2 = smoothdata(ssr2,2,'movmean',100);
        %         if strcmp(reg,'previoustotalliquid')
        %             r2=zscore(r2,[],2);
        %         end
        ssr2 = sr2(srtix,:);
        % ssr2(~sigix)=0;
        figure
        %         surf(xax,yax,ssr2,'EdgeColor','none')
        xlabel('Time (ms)')
        ylabel('Neuron #')
        %         zlabel('r2')
        sf = max(max(ssr2))/2;
        imagesc(xax,yax,ssr2,[0.00 .2])
        %         imagesc(xax,yax,r2,[0.00 sf])
        ...
        % ssbta(~sigix)=0;
        figure
        surf(xax,yax,ssbta,'EdgeColor','none')
        %         surf(xax,yax,bta,'EdgeColor','none')
        %
...
%
%
%                 [b,bint,r,rint,stats]= regress(fr,Xs); %stats=[r2 F p var];
%                 r2s(iR,ii) = stats(1);
%                 pvs(iR,ii) = stats(3);
%                 btas(iR,ii) = BetaNormalization(b(2),stst,fr);
%                 if shuff
%                     for iii = 1:100
%                         rndix = randperm(height(X));
%                         [b,bint,r,rint,stats]= regress(fr,X(rndix,:)); %stats=[r2 F p var];
%                         cr2(iR,ii,iii) = stats(1);
%                         cpv(iR,ii,iii) = stats(3);
%                         cbta(iR,ii,iii) = BetaNormalization(b(2),stmb(rndix),fr);
%
%                         [b,bint,r,rint,stats]= regress(fr,Xs(rndix,:)); %stats=[r2 F p var];
%                         cr2s(iR,ii,iii) = stats(1);
%                         cpvs(iR,ii,iii) = stats(3);
%                         cbtas(iR,ii,iii) = BetaNormalization(b(2),stst,fr);
%                     end
%                 end