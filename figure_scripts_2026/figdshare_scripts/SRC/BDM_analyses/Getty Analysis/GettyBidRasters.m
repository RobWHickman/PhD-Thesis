function [LMH]=GettyBidRasters(data_file_path_name,situations,bits,trials,win_lose_both)
LMH = [];
...
%         tr_cts = trial_rast;
%         trial_rast = Z_scores_DH(trial_rast,[650/bin:700/bin]);
% %         trial_rast = zscore(trial_rast,[],2);
%         trial_rast = MinMaxFS(trial_rast);
%         trial_rast(isnan(trial_rast))=0;
...
%                 xax = -pre+bin/2:bin:post-bin/2;
%                 plot_error_lines(strial_rast([lmh{iBid}],:),'SEM',xax,col(iBid,:));
%                 fr = nanmean(strial_rast([lmh{iBid}],:));
               
            end
            if ~isempty(fr)&& length(fr)==(pre+post)/bin
                FR(iBid,:) = fr;
                %                     SEM(iBid,:) = sem;
                %                 else
                %                     continue
        ...
        LongFigs
        savnam = strfix(['MonkBidRaster_',situation_names{situations},' | ',gdbits{iB},' | ',clust_nams{iC}]);
%         rmdir([pth,'MonkBidRasters']);
        savpth = We_want_dir_funk([pth,'MonkBidRasters_RmNS\']);
