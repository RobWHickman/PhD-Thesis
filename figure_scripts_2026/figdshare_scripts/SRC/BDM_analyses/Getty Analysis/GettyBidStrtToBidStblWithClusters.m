function [LMH]=GettyBidStrtToBidStblWithClusters(data_file_path_name,situations,bits,trials,win_lose_both)
...
%             alignment1 = [DS(gx).BidStartUp];
%             alignment2 = [DS(gx).BidStableUp];
            
            for iGx = 1:length(gx)%for each trial defined by goodix.
                alignment1 = [];  alignment2 = [];
                alignment1 = [DS(gx(iGx)).BidStartUp];
                alignment2 = [DS(gx(iGx)).BidStableUp];
                cont_alignment = [DS(gx(iGx)).FixationCrossUp];
                spks_per_trial = [DS(gx(iGx)).(clust_nams{iC})];
                trial_rast_tmp = TrialRaster(spks_per_trial,alignment1,0,alignment2-alignment1,bin);%generate a single-trial raster
                cont_trial_rast_tmp = TrialRaster(spks_per_trial,cont_alignment,pre,post,bin);%generate a single-trial control raster
                fr(iGx) = nanmean(trial_rast_tmp);
                cont_fr(iGx) = nanmean(trial_rast_tmp);
            ...
            LMH(ctr).BidStrtBidStbl(iBid,:) = FR;
            hold on
