...
for i=1:length(RES)
    ...
    [tr(i,1),~] = size(rst);
    sums(i,1)= sum(sum(rst));
    if sums(i,1)<75 || tr(i,1)<75 
        figure
        subplot(7,1,1:5)
        PlotTrueRaster(rst)
        title(num2str([i,sigix(i),tr(i)]))
        subplot(7,1,6:7)
        plot(mean(rst))
        ca
    end
...
%     [~,sx]=sort(pt);
    ...
    if zsc
        imagesc(xax,[1:length(sRES)],tc',[-.2 .4]);%[-.2 .3]
%         imagesc(xax,[1:length(sRES)],tc',[-2 10]);%[mn+((mx-mn)/6) mx/3])
    else
        imagesc(xax,[1:length(sRES)],tc',[mn mx/6])
    end
%     xlim([-npre npost])
    set(gca,'YDir','normal')
...
npost = 2000;
bgs=1;
iter = 25; % iterate by iter milliseconds
bw = 50; % bin width 
sigOnly = 0;
...
%     crst = double(sRES(iR).rast.FixationCrossUp);
    ...
    if bgs
        brst=rst-mean(crst(:,npre-1000:npre-50),2,'omitnan');
    end    
...
% ssr2 = smoothdata(r2(srtix,:),2,'gaussian',10);
% ssr2 = smoothdata(ssr2,1,'gaussian',10);
ssr2 = r2(srtix,:);
% ssr2(~sigix)=0;
figure
