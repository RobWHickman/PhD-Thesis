...
for i = 1:length(RES)
    if isnan(RES(i).rast.(sigbit))
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%
        if strcmp(sigbit,'WinLoseUp')
            mb=100-cb;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%
        pcb = double(RES(i).event.previouscomputerbid_same_RV(wlix));
        sit = double(RES(i).event.situations(wlix));
        wltr = double(RES(i).event.previouswinlose(wlix));
        tl = double(RES(i).event.previoustotalliquid(wlix));
        sb = double(RES(i).event.startingbid(wlix));
...
% % %     end
    ...
    mdl = fitlm(dv,iv);
    Reg_stats(iSt).p = mdl.Coefficients.pValue(2);
    Reg_stats(iSt).r2 = mdl.Rsquared.Ordinary;
    b=[];
    b = mdl.Coefficients.Estimate(2);
    Reg_stats(iSt).b = BetaNormalization(b,iv,dv);
    disp(['p = ',num2str(p),' | r2 = ',num2str(r^2)])
    title([r^2,p])
...
% % %     x=1:nbg;
    ...
    hold on
    plot(cAx,xax(xaxix),pv,'-','Color',col,'LineWidth',3)
    cAx.XLim=[0 100];
    cAx.YLim = yl;
    hold on
    disp('~~~~~~~~~~~~~~~~~~~~Weighted~~~~~~~~~~~~~~~~~~~~')
    ...
    ft='meta';
%     axis tight
    ti = ['All 3 sits chunked | ',monk,' | ',bit];
