function [LMH]=GettyMonkMinusCompBidDifferencesWithClusters(data_file_path_name,situations,bits,trials,win_lose_both)
...
for iC = 1:length(clust_nams)%for each cluster
    ...
    if frp < 0.01    
        ...
        for iB = 1:length(gdbits)%for each relevant bit     
            ...
            gi = ~ismember(double([DS(goodix).situation]),4);            
            bid_dif = (double([DS(goodix(gi)).MonkeyBid]))-(double([DS(goodix(gi)).ComputerBid]));
            
            if strcmp(win_lose_both,'win')
                bid_dif = bid_dif(bid_dif>0);
            elseif strcmp(win_lose_both,'lose')
                bid_dif = bid_dif(bid_dif<0);
            end
...
%             figure
            for iBid = 1:length(bid_lmh)
                gis = ismember([DS(goodix).situation],sit)...
                    ...
                    &ismember((double([DS(goodix).MonkeyBid]))-(double([DS(goodix).ComputerBid])),[bid_lmh{iBid}]);
                gx = goodix(gis);
...
%                 subplot(7,1,6:7)
%                 Peri_Event_Firing_Rate(trial_rast,bin,pre)
%                
%                 figure
%                 subplot(7,1,1:5)
%                 Imagesc_for_rast(zFRrast)
%                 subplot(7,1,6:7)
%                 plot(nanmean(zFRrast))
%                 tilefigs
%                 
%                 waitforbuttonpress
%                 ca
