...
%     load([td,'Vic_cells_sits_1  2  3.mat'])
    savDir = We_want_dir_funk([d,'Schultz_Lab\BDM_Data\Manuscript\Misc Figs\Rasters by frac smooth\Vic\',dt,'\sit_',ts,'\']);
...
%         edgs = quantile(mb,q(2:end-1)/100);
%         edgs = [0,edgs,101];

        edgs(1)=0; edgs(end)=100;
        [~,~,bix] = histcounts(mb,edgs);
%         [~,~,bix] = histcountsEvenBins(mb,5);
...
%                 edgs = [0,edgs,101];
            ...
            % % %edgs = linspace(mnmb,mxmb,splt+1);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % edgs = quantile(mb,[.25 .75]);
            %edgs = quantile(mb,[.33 .66]);
            %edgs = [0,edgs,101];
...
%                 sitRast_bdqnt = zscore(sitRast_bdqnt,[],2);
            ...
            g=gca;
            if max(max(alltrc))>min(min(alltrc))
