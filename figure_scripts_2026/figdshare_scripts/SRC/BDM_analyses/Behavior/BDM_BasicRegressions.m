...
badvars = {'budget' 'monkey_bid' 'budget_liquid' 'date' 'error' 'error_timeout' 'reward_magnitude'...
    'finish' 'free_reward' 'results' 'subtask' 'time' 'task_failure' 'juice' 'water'...
    'rewarded' 'unrewarded' 'rewarded' 'paid' 'previous_CB' 'previous_trial_error' 'monkey_ID'};
...
for iP = 1:width(BDM)
    ...
    %     pv = polyval(pf,x);
    %     plot(X,pv)
    statstr = sprintf(' \n beta = %.2g | R2 = %.2g | p = %.2g',bn,stats(1),stats(3));
    titstr = [varname,statstr];
    title(titstr,'FontSize',10,'FontWeight','normal','Interpreter','none')
