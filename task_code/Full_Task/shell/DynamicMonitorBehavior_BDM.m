function [flipwhen, errorInterrupt] = DynamicMonitorBehavior_BDM(rate,runTime)
...
%% Set-up stimulus strings:
Offer_Reward    = TP.Reward.BDM.Position;
Offer_BarBorder = TO.Rewards.Water.BDM.Border;
Offer_BidBar    = TO.Rewards.Water.BDM.Bar;
Offer_Scale     = TO.Rewards.Water.BDM.Scale;
Offer_FScale    = TO.Rewards.Water.BDM.FineScale;
Marker_Pos      = TO.Rewards.Water.BDM.MMPos;
Bar_Top         = [TO.Params.BDM.BarMin, (TO.Params.BDM.BarMin+TO.Stimuli.MMarker.C_Height)];
Bar_Bottom      = [(TO.Params.BDM.BarMax-TO.Stimuli.MMarker.C_Height), TO.Params.BDM.BarMax];
...
Static          = [Offer_BarBorder Offer_BidBar Offer_Reward Offer_FScale Offer_Scale Target];
StopEdit        = 0;
MarkerMoved     = false;
...
%     if TP.BDM.Bidlock ~= 1
%         if Evnt.joyY < 0        % Going down
%             BiasFactor = 1;
%         elseif Evnt.joyY > 0    % Going Up
%             BiasFactor = 1;
%         else % Zero velocity
%             BiasFactor = 1;
%         end
%     end
    ...
    if TP.BDM.Bidlock ~= 1
        Marker_Pos([2 4]) = Marker_Pos([2 4]) - (TP.BDM.Joy.Gain*Evnt.joyY);
    elseif TP.BDM.Bidlock == 1 && StopEdit == 0
        Marker_Pos(2) =  Marker_Pos(2) - 4;             % We enlarge the marker to indicate that the bid has been locked, and we do not move it.
        Marker_Pos(4) =  Marker_Pos(4) + 4;
        StopEdit = 1;                                   % This means we only modulate the marker position to show bid-locking once (otherwise the marker would grow!).
    end

%     if ~MarkerMoved
%         if Marker_Pos(2) > StartPos(2)+2 | Marker_Pos(4) < StartPos(4)-2
%             MarkerMoved = true;
%         end
%     end
    
    % NEED TO CONSIDER THAT THIS WILL MEAN NO EXPANSION ABOVE/BELOW MIN/MAX
    if Marker_Pos(2) < TO.Params.BDM.BarMin                 % Marker over the top of bar.
        Marker_Pos([2 4]) = Bar_Top;
    elseif Marker_Pos(4) > TO.Params.BDM.BarMax             % Marker below the bottom of the bar.
        Marker_Pos([2 4]) = Bar_Bottom;
    end
    % NEW, DID IT MOVE OUT AND BACK IN TO STARTING POSITION??
    if ~TO.Rewards.Water.BDM.MMVar
        switch TO.Rewards.Water.BDM.RelMPos
            case 0
                if Marker_Pos(4) < TO.Params.BDM.BarMax - 4 % Marker moved over bottom by 4 pixels
                    MarkerMoved = true;
                end
            case 1
                if Marker_Pos(2) > TO.Params.BDM.BarMin + 4 % Marker moved under top by 4 pixels
                    MarkerMoved = true;
                end
        end
    end
    ...
    Screen('Flip',10);
    TP.BDM.MarkerHistory(loops,:) = Marker_Pos;    % Update the marker history matrix.
...
if strcmp(TP.BDM.AuctionType,'BDM_Forced')
    if (Marker_Pos(4)-4) < TO.Stimuli.BDMf.TargetPos(4) && (Marker_Pos(2)+4) > TO.Stimuli.BDMf.TargetPos(2)
...
eval(Static);
Screen('FillRect',10, TO.Rewards.Water.BDM.MMOnColor, Marker_Pos);
BidPoint        = Marker_Pos(4) - 4;
TO.Params.BDM.FP_BidPoint = Marker_Pos(4);
BidPixel        = TO.Params.BDM.BarMax - BidPoint;
TP.BDM.MBID     = BidPixel/TO.Params.BDM.BarRange;
Bid             = TP.BDM.MBID

switch TO.Rewards.Water.BDM.RelMPos
    case 0
        if TP.BDM.MBID <= 0.005 && ~MarkerMoved
            TP.BDM.MBID = NaN;
            TaskOp.Error.NoChoice   = 1;
            TaskOp.correct          = 0;
            errorInterrupt          = 1;
    ...
    case 1
        if TP.BDM.MBID >= 0.995 && ~MarkerMoved
            TP.BDM.MBID = NaN;
            TaskOp.Error.NoChoice   = 1;
            TaskOp.correct          = 0;
            errorInterrupt          = 1;
...
% 
global IO BehaveData TaskOp Tbl UserInfo TP
...
if IO.Input.joy.monitor == 1 && TP.BDM.Bidlock ~= 1,
    Evnt.joyY                       = GJPY;
    Evnt.joyX                       = GJPX;
    ...
    if currentRow >= WR
        exitrule                    = max(abs(BehaveData.Tbl((currentRow-TP.BDM.Joy.Window):currentRow,9)));
        if abs(exitrule) <= IO.Input.joy.Sensitivity_Threshold
          TP.BDM.Bidlock       = 1;
          disp('Joystick stabilised. Bid placed.')
        end
    end
