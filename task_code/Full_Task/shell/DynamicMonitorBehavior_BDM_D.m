function [flipwhen, errorInterrupt] = DynamicMonitorBehavior_BDM_D(rate,runTime)
...
s_RectWidth             = num2str(15); %TO.Stimuli.BDM.D_FrameWidth);
...
Static= [Offer_BarBorder Offer_BidBar Offer_BidBarFScale Offer_BidBarScale Offer_Reward Target];
Frac1 = (TO.Rewards.Water.BDM.VarID/TO.Params.BDM.D_nDivs);
Frac2 = ((TO.Rewards.Water.BDM.VarID-1)/TO.Params.BDM.D_nDivs);
JPos  = ((Frac1+Frac2)/2)*TO.Stimuli.BDM.BarHeight;
%% Actual timer
while stopBehaviorTimer==0
    ...
    %% MoveMarker:
    if TP.BDM.Bidlock ~= 1
        JPos = JPos + (TP.BDM.Joy.Gain*Evnt.joyY);
    end

    if JPos < 0
        JPos = 0;
    elseif JPos > TO.Stimuli.BDM.BarHeight
        JPos = TO.Stimuli.BDM.BarHeight;
    end

    RelPos = DiscretePos(JPos);
    
    TO.Rewards.Water.BDM.DMMPos     = TO.Stimuli.BDM.D_PosMat(RelPos,:);
    ...
    Screen('Flip',10);
    TP.BDM.MarkerHistory(loops,:) = TO.Rewards.Water.BDM.DMMPos;    % Update the marker history matrix.
...
eval(Static);
Screen('FrameRect',10, TO.Rewards.Water.BDM.MMOnColor, TO.Rewards.Water.BDM.DMMPos, TO.Stimuli.BDM.D_FrameWidth);
TP.BDM.MBID     = RelPos/TO.Params.BDM.D_nDivs;
Bid             = TP.BDM.MBID
