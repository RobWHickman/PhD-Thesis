function [flipwhen, errorInterrupt] = DynamicMonitorBehavior_BCs(rate,runTime)
...
Marker_Pos              = TaskOp.BC.CenterPos;
TP.BCs.ChoiceDone       = 0;
s_WindowNumber          = num2str(VisParam.scr_handle);
WindowSize              = TO.Params.BCs.OfferWidth/2;
...
while stopBehaviorTimer==0
    ...
    %% MoveMarker:
        ...
        JoyPos = Evnt.joyX;
        Marker_Pos = Marker_Pos + (TP.BCs.Joy.Gain*JoyPos);
        if Marker_Pos < 25
            Marker_Pos = 25;
        ...
        MarkerP = [Marker_Pos-25, 540-25, Marker_Pos+25, 540+25];
    ...
    % END OF EDIT
    if Marker_Pos < (TP.BCs.LeftLimit+WindowSize) && Marker_Pos > (TP.BCs.LeftLimit-WindowSize)           % Marker In left-choice zone.
        TP.BCs.ChoiceSide = 1;
        TP.BCs.ChoiceDone = 1;
        s_MarkerCol = num2str([250 0 0]);
        if TP.BCs.FractalSide  == 1 % Fractal offer is on left, left side chosen
            TP.BCs.ChoiceType = 1;
        else
            TP.BCs.ChoiceType = 2;
        end
        Cover = TO.Stimuli.BCs.CoverRight;
    elseif Marker_Pos > (TP.BCs.RightLimit-WindowSize) && Marker_Pos < (TP.BCs.RightLimit+WindowSize)         % Marker In right-choice zone.
        TP.BCs.ChoiceSide = 2;
        TP.BCs.ChoiceDone = 1;
        s_MarkerCol = num2str([250 0 0]);
        if TP.BCs.FractalSide  == 2 % Fractal offer is on right, right side chosen
            TP.BCs.ChoiceType = 1;
        else
            TP.BCs.ChoiceType = 2;
        end
    ...
    TP.BCs.MarkerHistory(loops,:) = Marker_Pos;    % Update the marker history matrix.
...
preview = peekdata(ExtDevice.aiObject, 4);% Preview the last 3 samples of the aiObject.
preview = preview(:,1);
joy     = (mean(preview) - 0.35); % Take the average sampled, ignoring samples outside the IQR.
...
preview = preview(:,2);
joy     = -(mean(preview)+0.01);     % Take the average sampled, ignoring samples outside the IQR.
if abs(joy) <= IO.Input.joy.Sensitivity_Threshold;
    joy = 0;
end
