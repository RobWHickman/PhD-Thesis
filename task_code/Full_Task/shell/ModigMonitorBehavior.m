function [] = ModigMonitorBehavior(varargin)
...
if IO.Input.eye.monitor,
    ...
    if ~isempty(TaskOp.eye.req)
        ...
        switch TaskOp.eye.req
            ...
            case 'choiceFixation'
                % NOTE, after acquiring don't let go?
                eyeIn = zeros(2,1);
...
%        disp(sprintf('fixHolder at: %g',fixHolder))
...
%         disp(sprintf('Executed  %g after hold starts',elTime))
        % execute it
        if size(toBeExec,2)==2
            eval(toBeExec{2});
        else
...
%         % Rotate x and y around center if camera placed at an angle. 
%         % clockwise rotation is positive 'AdjustAngle'
%         AdjAng=10;
%         eye.x=((cos((atan((eye.y)/(eye.x)))-(AdjAng/180*pi)))*sqrt((eye.y)^2+(eye.x)^2));
%         eye.y=((sin((atan((eye.y)/(eye.x)))-(AdjAng/180*pi)))*sqrt((eye.y)^2+(eye.x)^2));
%         % Mirror in origin for negative X values
%         if eye.x<0;
%             eye.x=-eye.x;eye.y=-eye.y;
%         end
end
