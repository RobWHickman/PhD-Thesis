function [flipwhen, errorInterrupt] = DynamicMonitorBehavior_BDMTP(rate,runTime)
...
Bar_Top         = [TO.Params.BDM.BarMin, (TO.Params.BDM.BarMin+TO.Params.BDM.MMHeight)];
Bar_Bottom      = [(TO.Params.BDM.BarMax-TO.Params.BDM.MMHeight), TO.Params.BDM.BarMax];
Static          = [Offer_BarBorder Offer_BidBar Offer_FScale Offer_Scale];
StopEdit        = 0;
TargetPos       = TO.Stimuli.BDM.TargetPos;
%% Actual timer
while stopBehaviorTimer==0
    ...
    elseif TP.BDM.Bidlock == 1 && StopEdit == 0
        Vis_Pos(2) =  Marker_Pos(2) - 4;             % We enlarge the marker to indicate that the bid has been locked, and we do not move it.
        Vis_Pos(4) =  Marker_Pos(4) + 4;
    ...
    eval(Static);
    if Marker_Pos(2) > TargetPos(2) && Marker_Pos(4) < TargetPos(4)
        TargetCol = TO.Stimuli.BDM.TargetOnCol;
        Success   = 1;
%         InTarVec(loops,1) = 1;
    else
        TargetCol = TO.Stimuli.BDM.TargetCol;
        Success   = 0;
%         InTarVec(loops,1) = 0;
    end
    
    Screen('FillRect',10, TargetCol, TargetPos);
    Screen('FillRect',10, TO.Rewards.Water.BDM.MMOnColor, Vis_Pos);
    Screen('Flip',10);
    
    % COMPLEX RULE: LOCK IF IN TARGET AREA FOR x amount of time.
%     
%     if loops > TP.BDM.Joy.Window
%         Tester = sum(InTarVec(loops-TP.BDM.Joy.Window:loops,1));
%         if Tester >= TP.BDM.Joy.Window
%             TP.BDM.Bidlock = 1;
%         end
%     end
    
    TP.BDM.MarkerHistory(loops,:) = Marker_Pos;
...
if IO.Input.joy.monitor == 1 && TP.BDM.Bidlock ~= 1,
    ...
    if currentRow >= WR
        ...
        if abs(exitrule) <= 0.04 %IO.Input.joy.Sensitivity_Threshold
          TP.BDM.Bidlock       = 1;
