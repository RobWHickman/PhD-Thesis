...
if UserInfo.lab_connection
    if strcmpi(targetEpoch,'WaterDelivery') || strcmpi(targetEpoch,'WaterDelivery_BC')
            ...
            if strcmp(TP.BDM.CurrencyType,'W')                              % USE ORANGE/BUDGET SOLENOID.
    ...
    elseif strcmpi(targetEpoch,'JuiceDelivery') || strcmpi(targetEpoch,'JuiceDelivery_BC')
            if strcmp(TP.Reward.Type,'Risky')                               % USE BLUE/WATER SOLENOID.
                if targetInMl > 0
                    jCurTime = (PolyJuice1OT(targetInMl))/1000;
                else
                    jCurTime = 0;
                ...
                curLine = ExtDevice.outputDio.juice1.Index;
            elseif strcmp(TP.Reward.Type,'Water')                           % USE GREEN/JUICE SOLENOID.
                if targetInMl > 0
                    jCurTime = (PolyJuice2OT(targetInMl))/1000; % Was different polyjuice
                else
                    jCurTime = 0;
                end
                curLine = ExtDevice.outputDio.juice3.Index;
            elseif strcmp(TP.Reward.Type,'Orange')                          % USE GREEN/JUICE SOLENOID.
                if targetInMl > 0
                    jCurTime = (PolyJuice2OT(targetInMl))/1000;
                else
                    jCurTime = 0;
                end
                curLine = ExtDevice.outputDio.juice3.Index;
            elseif strcmp(TP.Reward.Type,'Learning')                        % USE BLUE/WATER SOLENOID.
                if targetInMl > 0
                    jCurTime = (PolyJuice1OT(targetInMl))/1000;
                else
                    jCurTime = 0;
